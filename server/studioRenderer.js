...
export async function renderStudioSession(job, onProgress = () => {}) {
  ...
  const musicPath = path.join(root, 'public', 'audio', 'music', musicNames[config.musicTrackId] || musicNames.zen_432hz);
  ...
  const musicVolume = Math.max(0, Math.min(1.5, renderable(config.musicVolume, 0.9))) * 1.15;
